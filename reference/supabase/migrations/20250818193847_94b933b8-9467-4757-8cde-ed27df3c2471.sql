...
-- 2) RLS на messages: заменить политики

drop policy "Employees can view messages by can_access_chat" on public.messages;
create policy "Employees can view messages by can_access_chat"
  on public.messages
  for select
  using (public.can_access_chat(chat_id));

drop policy "Employees can insert messages by can_access_chat" on public.messages;
...
drop policy "Employees can view chats by can_access_chat" on public.chats;
...
drop policy "Users can view active supergroups" on public.telegram_supergroups;
...
-- 5) Ослабить RPC для получения чатов по супергруппе
...
-- 6) Обновить функцию доступа для edge-функции
...
-- 8) Обновить политики для message_reads и typing_indicators

drop policy "Employees can mark read if can_access_chat" on public.message_reads;
...
drop policy "Employees can set typing indicators if can_access_chat" on public.typing_indicators;
...
drop policy "Employees can update their typing indicators" on public.typing_indicators;
create policy "Employees can update their typing indicators"
  on public.typing_indicators
  for update
  using (auth.uid() = user_id);

drop policy "Employees can view typing indicators if can_access_chat" on public.typing_indicators;
