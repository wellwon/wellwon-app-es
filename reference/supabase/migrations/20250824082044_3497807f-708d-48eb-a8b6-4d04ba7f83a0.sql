-- Enhanced reply integrity and cleanup script for WellWon chat system (fixed version)
-- This migration adds constraints, indexes and cleanup for reply_to_id integrity

-- 1. Add indexes for better reply message performance (without CONCURRENTLY)
...
-- 2. Create system_logs table if it doesn't exist
...
-- 3. Create security definer function for checking developer status
CREATE OR REPLACE FUNCTION public.is_user_developer()
RETURNS boolean AS $$
BEGIN
  RETURN COALESCE(
    (SELECT developer FROM profiles WHERE user_id = auth.uid()),
    false
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER STABLE;

-- 4. Create policy for system logs using the security definer function
DROP POLICY IF EXISTS "Developers can view system logs" ON public.system_logs;
CREATE POLICY "Developers can view system logs"
ON public.system_logs
FOR SELECT
USING (is_user_developer());

-- 5. Create function to validate reply_to_id integrity
...
-- 6. Create trigger for reply validation
...
-- 7. ONE-TIME CLEANUP: Remove orphaned reply_to_id references
...
cleanup_log AS (
  ...
  SELECT 
    ...
    jsonb_build_object(
      'total_cleaned', (SELECT COUNT(*) FROM orphaned_replies),
      'cleanup_timestamp', NOW()
    )
  WHERE EXISTS (SELECT 1 FROM orphaned_replies)
  RETURNING id
)
...
-- 8. Create function for periodic cleanup (can be called manually)
