-- Update the handle_new_user function to use user_type from metadata
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NULLIF(NEW.raw_user_meta_data->>'user_type', ''), 'client')::user_type -- use metadata user_type or default to client
  );
  RETURN NEW;
END;
$function$