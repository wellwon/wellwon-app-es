...
-- 5. Create security definer function for checking developer status
...
-- 6. Create policy for system logs using the security definer function
CREATE POLICY "Developers can view system logs"
ON public.system_logs
FOR SELECT
USING (is_user_developer());

-- 7. Create trigger for reply validation
...
-- 8. ONE-TIME CLEANUP: Remove orphaned reply_to_id references
...
-- 9. Create function for periodic cleanup (can be called manually)
