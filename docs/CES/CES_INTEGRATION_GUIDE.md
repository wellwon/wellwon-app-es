# Compensating Event System (CES) - Integration Guide

## External Change Detection for Event-Sourced Systems

**Version:** 1.1
**Date:** November 2025
**Pattern:** Compensating Event System (CES) + Transactional Outbox
**Terminology:** Greg Young - "Compensating Events"
...
**Compensating Event System (CES)** - a pattern where PostgreSQL triggers detect external data modifications (bypassing the application) and generate **compensating events** to maintain system consistency and audit trail.

| Term | Definition |
|------|------------|
| **Compensating Event** | An event that records an external change that bypassed the normal command flow |
| **CES Trigger** | PostgreSQL trigger that fires only for external changes (`pg_trigger_depth() = 0`) |
| **CES Handler** | `@sync_projection` decorated handler that processes compensating events |
...
BEGIN
    ...
    SELECT
        ...
        ROUND(AVG(EXTRACT(EPOCH FROM (COALESCE(e.published_at, NOW()) - e.created_at)) * 1000), 2) AS avg_latency_ms,
        ROUND(MAX(EXTRACT(EPOCH FROM (COALESCE(e.published_at, NOW()) - e.created_at)) * 1000), 2) AS max_latency_ms,
        COUNT(*) FILTER (WHERE e.status = 'pending') AS pending_count,
        COUNT(*) FILTER (WHERE e.status IN ('failed', 'dead_letter')) AS failed_count
    FROM event_outbox e
