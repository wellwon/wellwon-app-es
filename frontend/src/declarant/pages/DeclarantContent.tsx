...
const DeclarantContent: React.FC = () => {
  ...
  // Current reference ID for nested navigation within references
  ...
  const pushToHistory = (item: NavigationHistoryItem) => {
    setNavigationHistory(prev => {
      // Don't push duplicate entries
      const last = prev[prev.length - 1];
      if (last &&
          last.section === item.section &&
          last.viewMode === item.viewMode &&
          last.packageId === item.packageId &&
          last.referenceId === item.referenceId) {
        return prev;
      }
      return [...prev, item];
    });
  ...
  // Navigation helper: go back one step
  const goBack = () => {
    if (navigationHistory.length <= 1) {
      // Can't go back further, stay on dashboard
      return;
    }
    ...
    if (previous.section === 'references') {
      ...
      setActiveReferenceId(previous.referenceId || null);
      navigateToDeclarantList();
    } else if (previous.section === 'dashboard') {
      setActiveSection('dashboard');
      setActiveReferenceId(null);
      if (previous.viewMode === 'package' && previous.packageId) {
        navigateToPackage(previous.packageId);
  ...
  // Section change handler with history tracking
  const handleSectionChange = (newSection: string) => {
    if (newSection !== activeSection) {
      pushToHistory({ section: newSection, viewMode: 'list', referenceId: null });
  ...
  // Reference navigation handler
  const handleReferenceNavigate = (referenceId: string | null) => {
    pushToHistory({ section: 'references', referenceId });
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  pushToHistory({ section: 'dashboard', viewMode: 'package', packageId: 'new-tt' });
              ...
              <button
                onClick={() => {
                  ...
                  pushToHistory({ section: 'dashboard', viewMode: 'package', packageId: 'new-ek' });
              ...
              <button
                onClick={() => {
                  ...
                  pushToHistory({ section: 'dashboard', viewMode: 'package', packageId: 'new-im' });
